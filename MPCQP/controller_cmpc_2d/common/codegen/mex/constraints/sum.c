...
void sum(const emlrtStack *sp, const real_T x_data[], const int32_T x_size[3],
         real_T y_data[], int32_T y_size[3])
{
  ...
  if ((x_size[0] == 0) || (x_size[2] == 0)) {
    sz_idx_0 = (int8_T)x_size[0];
    sz_idx_2 = (int8_T)x_size[2];
    y_size[0] = sz_idx_0;
    y_size[1] = 1;
    y_size[2] = sz_idx_2;
    k = sz_idx_0 * sz_idx_2;
    ...
    for (i = 0; i < npages; i++) {
      ...
      for (j = 0; j <= vstride; j++) {
        y_data[ypageoffset + j] = x_data[k + j];
      }
